...
describe('Config', () => {
  ...
  describe('readConfig', () => {
    ...
    it('should throw an error if sheet is missing', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (global as any).SpreadsheetApp = {
        getActiveSpreadsheet: jest.fn().mockReturnValue({
      ...
      expect(() => Config.readConfig('NonExistentSheet')).toThrow(
        'Sheet "NonExistentSheet" not found.'
      );
    });

    it('should throw an error if active spreadsheet is missing', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (global as any).SpreadsheetApp = {
        getActiveSpreadsheet: jest.fn().mockReturnValue(null),
      };

      expect(() => Config.readConfig()).toThrow('No active spreadsheet found.');
    });

    it('should throw an error if range/rows is missing or empty', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (global as any).SpreadsheetApp = {
        getActiveSpreadsheet: jest.fn().mockReturnValue({
          getSheetByName: jest.fn().mockReturnValue({
      ...
      expect(() => Config.readConfig()).toThrow(
        'No configuration values found in sheet "Config".'
      );
    });
